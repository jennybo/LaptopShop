...
public class CartServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
                ...
                case "sub":
                    if(cart.getCartItems().containsKey(idProduct)){
                        if(cart.getCartItems().get(idProduct).getQuantity()<=1)
                            ...
                            int oldQuantity = cart.getCartItems().get(idProduct).getQuantity();
                            Item item = cart.getCartItems().get(idProduct);
                            cart.subToCart(idProduct, item);
                        }
                    }
                    break;
            }
            ...
            e.printStackTrace();
            response.sendRedirect(Constants.LINK_ROOT+"/checkout.jsp");
        }
